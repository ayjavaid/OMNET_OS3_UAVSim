...
void PingApp::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        WATCH(numPongs);
        //cout << "loss count :" << lossCount<< "sentCount: " <<sentCount ; // added by Farha for debugging on 05.26.2013
        // references
        timer = new cMessage("sendPing");
        cout<< "timer= "<< timer << "count="<<count<< endl;
    }
    else if (stage == 3)
    {
        // startup
...
{
    cout <<"stop sending ping request:" ;
    pid = -1;
    lastStart = -1;
    sendSeqNo = expectedReplySeqNo = 0;
...
{
    //cout << "destAddr is " << par("destAddr").stringValue() << endl ;
...
void PingApp::sendPingRequest()
{
    if (destAddr.isUnspecified())
    {
       ...
       // cout << "Source Address" << srcAddr; //added by farha  5.26.2014
