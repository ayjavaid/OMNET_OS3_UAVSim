...
bool IPvXAddressResolver::tryResolve(const char *s, IPvXAddress& result, int addrType)
{
    ...
    //cout << "I am here nextsep: " << nextsep << endl ;
    if (!nextsep)
        nextsep = endp;
    // cout << "Parent Module: "<< p <<endl ;              //added by Farha Jahan for debugging 05.23.2014
    ...
    // find module
    //cout << "Destination address :"<< modname.c_str()<< endl;              //added by Farha Jahan for debugging 05.23.2014
    cModule *mod = simulation.getModuleByPath(modname.c_str());
    //cout << "Destination address :"<< mod << endl;                         //added by Farha Jahan for debugging 05.23.2014
    if (!mod)
