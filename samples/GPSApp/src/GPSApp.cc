...
EXECUTE_ON_STARTUP(
    ...
    e->insert(GPSApp::ONCE, "once");
    e->insert(GPSApp::PER_BURST, "perBurst");
    e->insert(GPSApp::PER_SEND, "perSend");
);

Define_Module(GPSApp);

int GPSApp::counter;

simsignal_t GPSApp::sentPkSignal = registerSignal("sentPk");
simsignal_t GPSApp::rcvdPkSignal = registerSignal("rcvdPk");
simsignal_t GPSApp::outOfOrderPkSignal = registerSignal("outOfOrderPk");
simsignal_t GPSApp::dropPkSignal = registerSignal("dropPk");
...
void GPSApp::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        timerNext = new cMessage("GPSAppTimer");

    }
}

IPvXAddress GPSApp::chooseDestAddr()
{
...
GPSPacket *GPSApp::createPacket()
{
    char msgName[32];
    sprintf(msgName, "GPSAppData-%d", counter++);
    ...
    payload->setDistanceArraySize(20);
    Sat = dynamic_cast<SatSGP4Mobility*>(this->getParentModule()->getSubmodule("mobility", 0));
              //cout<< "Sat :" << this->getParentModule()->getSubmodule("mobility", 0);

    //cout<<"Satellite packet";
    payload->addPar("satId")=this->getParentModule()->getIndex();   // satellite index
    payload->addPar("Xcoord")= Sat->getPositionX();
    payload->addPar("Ycoord")= Sat->getPositionY();
    payload->addPar("Attack")=83 ;
    //payload->addPar("Latitude")= Sat->getLatitude();
    //payload->addPar("Longitude")= Sat->getLongitude();
    //payload->addPar("Altitude")=Sat->getAltitude();
    //payload->addPar("Elevation")= Sat->getElevation(Sat->getLatitude(),Sat->getLongitude(),Sat->getAltitude());
    //payload->addPar("Azimuth")=Sat->getAzimuth(Sat->getLatitude(),Sat->getLongitude(),Sat->getAltitude());


   // cout<< "Satellite X: "<< Sat->getPositionX()<<" , Y= "<<Sat->getPositionY()<<endl;
   // cout<< "Satellite Current position"<< Sat->getCurrentPosition()<<endl;
    int z=0;
    double dist=0.0;
                    ...
                    Host = dynamic_cast<LinearMobility*>(this->getParentModule()->getParentModule()->getSubmodule("host", z)->getSubmodule("mobility", 0));
                        //cout<< "Host :" << Host;
                           ...
                           // cout<<"Host's current position 1: "<<Host->getCurrentPosition()<<endl;
                            ...
                            dist=Host->getCurrentPosition().distance(Sat->getCurrentPosition());
                            payload->setDistance(z, dist);
                            z++;
                            //cout<<"Nearest satellite is at: "<<dist;

                }
     // cout<< "Host :" <<simulation.getModule(i)<<"     parent module"<<this->getParentModule()->getParentModule()->getSubmodule("host", 0)<<"    "<<endl;             //->getIndex();
            }
      //cout<<"payload :"<< payload;
    return payload;
}

void GPSApp::processStart()
{
...
void GPSApp::processStop()
{
    socket.close();
}

void GPSApp::handleMessageWhenUp(cMessage *msg)
{
...
void GPSApp::processPacket(GPSPacket *pk)
{
    /*#if OMNETPP_VERSION < 0x500
    for (int i=0; i<=simulation.getLastModuleId(); i++)
    #else
    ...
    if (pk->hasPar("sourceId") && pk->hasPar("msgId"))
    {
        ...
        if(count<=3)
            ...
            if(lock==0)
            {
                ...
                //cout<<"--------Distance of the satellite and Host or attack host "<<a<<": "<<pk->getDistance(index)<<"-----------"<<endl;
            ...
            else
            {
                ...
                for(p=0; p<=2;p++)
                {
                    ...
                    countSats[count] = satId;
                    X[count]= (double)pk->par("Xcoord");
                    Y[count] = (double)pk->par("Ycoord");
                    distance[count] = pk->getDistance(index);
                    for(int i=0; i<=count;i++)
                       {
                            if((countSats[count]==countSats[i]) && count!=i)
                            {
                             countSats[count]=0;
                             X[count]= 0;
                             Y[count] = 0;
                             distance[count]=0;
                             count--;
                             //cout<<"Difference between timer and simulation time"<<simTime()-timer;
                             if((simTime()-timer)>10)
                                 lock=0;
                             break;
                             }
                ...
                if(p<=2)
                {
                   // cout<<"value of count: "<<count<<endl;;
                    //cout<<countSat[count]<<", ";
                    //cout<<"Satellite Id Index["<<count<<"]: "<< countSats[count]<<endl;
                   // cout<<"Satellite Id X["<<count<<"]: "<< X[count]<<endl;
                   // cout<<"Satellite Id Y["<<count<<"]: "<< Y[count]<<endl;
                   // cout<<"Satellite Id distance["<<count<<"]: "<< distance[count]<<endl;
                   // cout<<"Host index: "<<index<<endl;
                    ...
                    if(count==3)
                    {
                       ...
                       // q++;
                        ...
                        timer=simTime();
                        //cout<<"Simulation Time "<<timer<<endl;
                        count=0;
                        lock=1;
...
double GPSApp::determinant(double a[][2], int n)
{
   int i,j,j1,j2;
   double det = 0;
...
*/
void GPSApp::CoFactor(double a[][2],int n,double b[][2])
{
   int i,j,ii,jj,i1,j1;
   double det;
   double c[2][2];
...
*/
void GPSApp::Transpose(double a[][2],int n)
{
   int i,j;
   double tmp;
...
void GPSApp::generateBurst()
{
    ...
    //cout<<"Destination Address 2 "<<destAddr;
...
void GPSApp::finish()
{
    ...
    cout << "--------------------------------------------------------" << endl; //        cout <<"\t" <<this->getParentModule()<<endl;
    cout << "\t" << getFullPath() << endl;
    cout << "--------------------------------------------------------" << endl;
    cout <<"Total received from Sat: "<< cntS <<"\t" <<"Total received from AH: "<<cntA<<endl;
    cout <<"Total number of times it Localized: " << cntL <<endl;
    cout <<"Total sent: "<<  numSent<<"\t\t\t"<<"Total received "<<  numReceived<<endl;
    cout <<"Total deleted "<<  numDeleted<<"\t\t\t"<<"Total duplicated "<<  numDuplicated<<endl;
    cout << "--------------------------------------------------------" << endl<<endl;
    ApplicationBase::finish();
}

bool GPSApp::handleNodeStart(IDoneCallback *doneCallback)
{
...
bool GPSApp::handleNodeShutdown(IDoneCallback *doneCallback)
{
    if (timerNext)
