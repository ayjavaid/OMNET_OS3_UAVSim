...
{
    CHECK(fprintf(f, "E # %" LL "d t %s m %d ce %" LL "d msg %ld\n", eventNumber, SIMTIME_STR(simulationTime), moduleId, causeEventNumber, messageId));
}

void EventLogWriter::recordKeyframeEntry_p_c_s(FILE *f, int64 previousKeyframeFileOffset, const char * consequenceLookaheadLimits, const char * simulationStateEntries)
{
    CHECK(fprintf(f, "KF p %" LL "d c %s s %s\n", previousKeyframeFileOffset, QUOTE(consequenceLookaheadLimits), QUOTE(simulationStateEntries)));
}
...
{
    CHECK(fprintf(f, "CE id %ld tid %ld eid %ld etid %ld c %s n %s pe %" LL "d\n", messageId, messageTreeId, messageEncapsulationId, messageEncapsulationTreeId, QUOTE(messageClassName), QUOTE(messageName), previousEventNumber));
}
...
{
    CHECK(fprintf(f, "BS id %ld tid %ld eid %ld etid %ld c %s n %s pe %" LL "d\n", messageId, messageTreeId, messageEncapsulationId, messageEncapsulationTreeId, QUOTE(messageClassName), QUOTE(messageName), previousEventNumber));
}
...
{
    CHECK(fprintf(f, "CM id %ld tid %ld eid %ld etid %ld c %s n %s pe %" LL "d\n", messageId, messageTreeId, messageEncapsulationId, messageEncapsulationTreeId, QUOTE(messageClassName), QUOTE(messageName), previousEventNumber));
}
...
void EventLogWriter::recordCloneMessageEntry_id_tid_eid_etid_c_n_k_p_l_er_d_pe_cid(FILE *f, long messageId, long messageTreeId, long messageEncapsulationId, long messageEncapsulationTreeId, const char * messageClassName, const char * messageName, short messageKind, short messagePriority, int64 messageLength, bool hasBitError, const char * detail, eventnumber_t previousEventNumber, long cloneId)
{
    ...
    if (detail!=NULL)
        CHECK(fprintf(f, " d %s", QUOTE(detail)));
    CHECK(fprintf(f, " pe %" LL "d cid %ld\n", previousEventNumber, cloneId));
}
...
{
    CHECK(fprintf(f, "DM id %ld tid %ld eid %ld etid %ld c %s n %s pe %" LL "d\n", messageId, messageTreeId, messageEncapsulationId, messageEncapsulationTreeId, QUOTE(messageClassName), QUOTE(messageName), previousEventNumber));
}
